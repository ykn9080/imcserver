 ...
 */
exports.getNodeDataAtTreeIndexOrNextIndex = ({
    targetIndex,
    node,
    currentIndex,
    getNodeKey,
 ...
 * @return {number} count
 */
function getVisibleNodeCount({ treeData }) {
    const traverse = node => {
        if (
            !node.children ||
            node.expanded !== true ||
 ...
 */
function getVisibleNodeInfoAtIndex({
    treeData,
    index: targetIndex,
    getNodeKey
}) {
...
function find({
    ...
    const trav = ({ isPseudoRoot = false, node, currentIndex, path = [] }) => {
        ...
        // The pseudo-root is not considered inthe path
        const selfPath = isPseudoRoot ? [] : [...path, getNodeKey({ node, treeIndex: currentIndex })];
